...
const isAnyObject = (input: unknown): input is AnyObject =>
...
export type AsyncStore = ReturnType<ReturnType<typeof createAsyncStore>>
...
function createAsyncStore(options: Options) {
  ...
  >({
    ...
    for (const dep of [...deps, ...requiredDeps]) {
 ...
 * @example
 * ```ts
 * import { createApiStores } from 'nanorequest'
 *
 * const { createAsyncStore, createMutator } = createApiStores()
 * ```
 */
